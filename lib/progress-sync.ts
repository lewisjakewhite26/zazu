...
export async function fetchRemoteProgress(user: User): Promise<ProgressState> {
  ...
  const wordProgress: UserWordProgressLocal[] = (data ?? []).map((row: {
    word_id: string;
    alarm_completed_at: string | null | undefined;
    gym_completed_at: string | null | undefined;
    gym_mastery: number | null | undefined;
    gym_wrong_count: number | null | undefined;
    next_review_at: string | null | undefined;
  }) => ({
    wordId: row.word_id,
    alarmCompletedAt: row.alarm_completed_at ?? null,
    gymCompletedAt: row.gym_completed_at ?? null,
