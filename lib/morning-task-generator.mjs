...
export function enrichWordEntry(entry) {
  ...
  // Passage is hand-authored (see zazu-words schema / lib/word-spotting.ts) and
  // never regenerated from the round data, so preserve it if the entry already
  // has one -- otherwise re-running `npm run words:morning-tasks` would wipe it.
  if (entry.morningTask?.passage) {
    morningTask.passage = entry.morningTask.passage;
  }
