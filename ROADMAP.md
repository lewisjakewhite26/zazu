...
| — | **Fix post-purchase entitlement error** — `SubscriptionContext.tsx`'s `purchaseGold`/`restorePurchases`/`refreshEntitlement` all called `upsertUserEntitlement` after a successful RevenueCat result, which unconditionally throws in production builds (entitlements are meant to come only from the webhook) — so a paying user saw an error screen immediately after a successful charge. | **Done (2026-08-26)** — all three call sites now only call `upsertUserEntitlement` in `__DEV__`; production sets local `entitlement` state optimistically and lets the webhook write the real row. `GoldPaywallScreen.handlePurchase`'s existing `router.back()` on success now actually runs instead of being hijacked into the catch block. |
| — | **Harden Google Sign-In** — `Google.useIdTokenAuthRequest` in `SignInScreen.tsx` threw synchronously during render if the platform-required client ID was undefined, before the app's own `isConfigured` check ever ran. Not currently broken (client IDs are set and EAS-synced), but this project has had two documented EAS-env-drift incidents already, and this path had zero fallback — an instant crash screen instead of a degraded state. | **Done (2026-08-26)** — the hook always receives a defined client ID now (real value or a `GOOGLE_CLIENT_ID_FALLBACK` placeholder), so a missing/drifted env var can no longer crash it; `googleConfig.isConfigured` still keeps the button disabled/inert either way. |

### P2 — Everything else

| # | Task | Status |
|---|------|--------|
| 10 | Extract shared game logic (`COPY`, puzzle engine, `mapWordRow` into `lib/`) | **Done (2026-08-26), rescoped** — the literal item was stale: `COPY`/`mapWordRow` only ever existed in the unshipped legacy web app (`index.html`), and row-mapping was already centralized in `lib/supabase.ts` (`mapAlarmRow`/`mapGymRow`/`mapGymRounds`). The real remaining duplication was a Fisher-Yates `shuffle<T>` copy-pasted verbatim across `gym-modes.ts`, `daily-ritual.ts`, `literary-words.ts`, `puzzle-utils.ts`, `spelling-puzzle.ts`, and inline again in `morning-task.ts` — consolidated into `lib/shuffle.ts`. Also centralized a few duplicated hardcoded strings (`"Exit demo alarm"`, screen-reader announcements) into `constants/copy.ts`'s `a11y` block. |
...
| 42 | Fix `zazu-words.schema.json` — stale (said "Target: 100 words," missing `roots`/`introEtymology`/`morningTask` fields), unused by any script today, so harmless but misleading | **Done (2026-08-25)** — schema now requires and types `roots`/`introEtymology`/`morningTask` matching the live shape; validated against all 393 words in `zazu-words.json` with zero mismatches |
| 43 | Reconcile git remote / deploy pipeline — `git remote -v` was empty in the round-9 audit despite README describing GitHub→Vercel auto-deploy | **Resolved (2026-08-25)** — the "empty remote" claim was already stale: `origin` (`github.com/lewisjakewhite26/zazu`) is configured and has been all along. The real gap found this session: local `main` was **29 commits ahead of `origin/main`**, unpushed since `311fdd9` — meaning weeks of shipped work (all 10 word packs, snooze redesign, MCQ screen dedupe, RLS fixes, text-size sweep) never reached GitHub, so any GitHub→Vercel auto-deploy would've been serving a stale build. Pushed (fast-forward, `311fdd9..ba6070f`); repo and remote are now in sync. Whether Vercel's auto-deploy trigger actually fires on push is still unconfirmed — worth checking the Vercel dashboard next deploy. |
...
| 49 | **Privacy policy + Terms of Service** — no document exists anywhere in the repo, not hosted anywhere, not linked from Settings on mobile or web. This is a real submission blocker, not polish: both the App Store and Google Play require a privacy policy URL in the store listing, and Zazu collects an account (Supabase auth), handles payments (RevenueCat subscriptions), and stores user progress data, all of which need disclosing. | **Done (2026-08-26)** — now `landing/app/privacy/page.tsx` and `/terms/page.tsx` in the merged `landing/` Next.js site, hosted at `zazu.org.uk/privacy` and `/terms`, linked from a new "Legal" section in mobile Settings |
| 50 | **Accessibility statement** — not a hard store requirement, but no document exists and nothing links to one. Worth writing given the app is already designed to a "70-year-old user" readability bar (see the 2026-08-22 text-size audit). | **Done (2026-08-26)** — `landing/app/accessibility/page.tsx`, same hosting/linking as #49 |
...
15. ~~Write and host a Privacy Policy + Terms of Service (#49)~~ — **done 2026-08-26**, see #49. Drafted from what the app actually does (Supabase auth, RevenueCat entitlements, no analytics/tracking); worth a lawyer's review before treating it as fully compliant, but unblocks store submission today.
16. ~~Write an Accessibility statement (#50)~~ — **done 2026-08-26**, see #50.
17. **Set up `support@zazu.org.uk`** — the Privacy Policy, Terms of Service, and Accessibility Statement (#49/#50) all list this as the contact address, but the mailbox doesn't exist yet. Needs to actually receive mail before those docs are accurate. *(Needs the user's domain/email host access — not something I can do directly.)*
